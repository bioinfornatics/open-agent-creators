...
function parseCommon(argv: string[]): {
  command?: Command; forwarded: string[]; format: Format; quiet: boolean; help: boolean; error?: string;
} {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--quiet") quiet = true;
      ...
      if (value !== "text" && value !== "json") return { forwarded: [], format, quiet, help, error: "--format must be 'text' or 'json'" };
      format = value;
  ...
  if (candidate && candidate in COMMANDS) return { command: candidate as Command, forwarded: remaining, format, quiet, help };
...
function classifyExit(status: number | null, output: string): number {
  if (status === 0) return 0;
  if (/refusing to overwrite|operation not permitted|permission denied|\bEACCES\b|\bEPERM\b/i.test(output)) return 3;
  if (status === 2 || /ERR_PARSE_ARGS|unknown option|usage:/i.test(output)) return 2;
  return 1;
}
...
export function runCli(argv: string[] = process.argv.slice(2)): number {
  ...
  if (parsed.error) {
    if (parsed.format === "json") console.log(JSON.stringify({ ok: false, command: parsed.command ?? null, exitCode: 2, error: parsed.error }));
    else { console.error("error: " + parsed.error); console.error(GENERAL_HELP); }
    return 2;
  }
  if (!parsed.command) {
    if (parsed.format === "json") console.log(JSON.stringify({ ok: true, command: null, exitCode: 0, help: GENERAL_HELP }));
    else if (!parsed.quiet) console.log(GENERAL_HELP);
    return 0;
  }
  if (parsed.help) {
    const help = commandHelp(parsed.command);
    if (parsed.format === "json") console.log(JSON.stringify({ ok: true, command: parsed.command, exitCode: 0, help }));
    else if (!parsed.quiet) console.log(help);
    return 0;
  }
  ...
  if (parsed.format === "json") {
    ...
    if (stdout && (!parsed.quiet || exitCode !== 0)) process.stdout.write(stdout);
    if (stderr) process.stderr.write(stderr);
    if (spawnError) process.stderr.write("error: " + spawnError + "\n");
  }
  return exitCode;
}
