...
| Standalone skills | `.agents/skills/`, `~/.agents/skills/` | No — use `skill-creator` |
| Standalone agents | `.agents/agents/`, `~/.agents/agents/` | No — use `agent-creator` |
...
- Bundled skills route to `skill-creator`.
- Hooks and hook scripts route to `hook-creator`.
- Standalone custom agents route to `agent-creator`.
- Manifest, MCP, packaging, and whole-plugin validation stay in `plugin-creator`.
