...
export async function improveDescription(skillName, skillContent, currentDescription, evalResults, history, model, options = {}) {
...
async function main() {
    ...
    if (!values["eval-results"] || !values["skill-path"] || !values.model) {
        console.error("usage: improve_description.js --eval-results <file> --skill-path <dir> --model <name> [--history <file>] [--runner goose] [--verbose]");
        process.exit(2);
    }
    ...
    const evalResults = JSON.parse(readFileSync(resolve(values["eval-results"]), "utf-8"));
    let history = [];
    if (values.history) {
        history = JSON.parse(readFileSync(resolve(values.history), "utf-8"));
    }
    ...
    const newDescription = await improveDescription(name, content, currentDescription, evalResults, history, values.model, { runner: values.runner });
    if (values.verbose) {
