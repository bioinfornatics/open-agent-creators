...
function shouldExclude(relParts) {
    if (relParts.some((part, index) => EXCLUDE_DIRS.has(part) && !(part === "node_modules" && relParts[index - 1] === "vendor")))
        return true;
    if (relParts.length > 1 && ROOT_EXCLUDE_DIRS.has(relParts[1]))
        return true;
...
function validateOfflineBundle(skillPath) {
    ...
    if (!pkg.offlineBundle)
        return [];
    const missing = [];
    if (!isDir(join(skillPath, "dist")))
        missing.push("dist/");
    if (!existsSync(join(skillPath, "vendor", "manifest.json")))
        missing.push("vendor/manifest.json");
    if (!existsSync(join(skillPath, "THIRD_PARTY_NOTICES.md")))
        missing.push("THIRD_PARTY_NOTICES.md");
...
function collectFiles(current, out, parentDir) {
    for (const entry of readdirSync(current).sort()) {
        const full = join(current, entry);
        const relPath = full.slice(parentDir.length + 1);
        if (shouldExclude(relPath.split(/[\\/]/)))
            continue;
        if (isSymlink(full))
            throw new Error(`Refusing to package symbolic link: ${relPath}`);
        if (isDir(full))
            collectFiles(full, out, parentDir);
        else
            out.push(full);
    }
}
export function packageSkill(skillPathArg, outputDirArg) {
