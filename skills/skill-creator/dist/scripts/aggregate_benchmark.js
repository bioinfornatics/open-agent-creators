...
function loadRunResults(benchmarkDir) {
    ...
    evalDirs.forEach((evalName, evalIdx) => {
        ...
        const configPriority = (name) => name.includes("with_skill") ? 0 : name.includes("old_skill") || name.includes("without_skill") ? 1 : 2;
...
function aggregateResults(results) {
    ...
    for (const config of configs) {
        ...
        const times = runs.map((r) => r.time_seconds).filter((value) => typeof value === "number");
        const tokens = runs.map((r) => r.tokens).filter((value) => typeof value === "number");
        runSummary[config] = {
...
function generateMarkdown(benchmark) {
    ...
    const formatMetric = (value, digits = 0) => value === null || value === undefined ? "n/a" : value.toFixed(digits);
    lines.push(`| Time | ${formatMetric(aTime.mean, 1)}s ± ${formatMetric(aTime.stddev, 1)}s | ${formatMetric(bTime.mean, 1)}s ± ${formatMetric(bTime.stddev, 1)}s | ${delta.time_seconds ?? "—"} |`);
    const aTokens = aSummary.tokens ?? {};
    const bTokens = bSummary.tokens ?? {};
    lines.push(`| Tokens | ${formatMetric(aTokens.mean)} ± ${formatMetric(aTokens.stddev)} | ${formatMetric(bTokens.mean)} ± ${formatMetric(bTokens.stddev)} | ${delta.tokens ?? "—"} |`);
...
function main() {
    ...
    const benchmark = generateBenchmark(benchmarkDirArg, values["skill-name"], values["skill-path"]);
