...
function copySkill(skillPath, projectRoot) {
...
async function runSingleQuery(query, skillPath, timeoutMs, model, runner) {
...
export async function runEval(evalSet, skillPath, numWorkers, timeoutMs, options = {}) {
    const { runsPerQuery = 1, triggerThreshold = 0.5, model, runner, description: descriptionOverride, } = options;
    ...
    try {
        ...
        if (originalDescription !== description) {
            ...
            if ([">", "|", ">-", "|-"].includes(valueAfterColon)) {
                while (descriptionEnd < frontmatterEnd &&
        ...
        await mapLimit(jobs, numWorkers, async ({ item }) => {
            ...
            try {
                ...
                console.error(`Warning: query failed: ${error.message}`);
...
async function main() {
    ...
    let output;
    try {
        output = await runEval(evalSet, skillPath, Number(values["num-workers"]), Number(values.timeout) * 1000, {
