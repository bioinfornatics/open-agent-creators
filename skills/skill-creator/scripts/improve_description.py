...
def improve_description(
    ...
    prompt = f"""You are optimizing a portable Agent Skill description for a skill called "{skill_name}". A "skill" is sort of like a prompt, but with progressive disclosure -- there is a title and description that the agent sees when deciding whether to use the skill, and then if the agent uses the skill, it reads the .md file which has lots more details and potentially links to other resources in the skill folder like helper files and scripts and additional documentation or examples.
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    # SDK path did this as a true multi-turn; the command adapter is one-shot, so we
